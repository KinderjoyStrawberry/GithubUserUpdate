...
import com.myapp.githubuserupdate.databinding.FragmentFollowingBinding
import cz.msebera.android.httpclient.Header
import org.json.JSONArray
import org.json.JSONObject

class FragmentFollowing(private val user: User) : Fragment() {
    ...
    private fun showRecyclerList() {
        if (requireContext().resources.configuration.orientation == Configuration.ORIENTATION_LANDSCAPE) {
            binding.listFOLLOWING.layoutManager = GridLayoutManager(requireContext(), 2)
            binding.listFOLLOWING.adapter = adapter
